...
accfile<-"/Users/matthewnelsen/Documents/papers_reviews/papers/ants_for_doug/17nov2019_1/Formicidae/combined.updated.pruned.csv"
out.accfile<-"/Users/matthewnelsen/Documents/papers_reviews/papers/ants_for_doug/17nov2019_1/Formicidae/combined.updated.updated.pruned.csv"
...
for (locus in loci){
	...
	if(nrow(in.old.not.new)>0){
		...
		for(j in 1:nrow(in.old.not.new)){
			...
			#try this instead...was having problems with not getting accession number in acc column.
			in.old.not.new$Accession[j]<-orig.accfile[orig.accfile$Taxon %in% in.old.not.new$Taxon[j],locus]
		}
	}
