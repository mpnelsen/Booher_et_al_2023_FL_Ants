...
grab.acc.nos.from.clade<-function(clade.to.search,locus,search.terms,excludes=NULL){
	...
	if(nrow(search.res)>=40000){
		cat(paste("\t\t\t...", locus, " SEARCH LIMIT REACHED (40,000 accessions) - SEARCH HALTED = try searching a smaller clade...\n", sep=""))
	...
	#so subset and make sure things like this are removed
		...
		search.res<-search.res[!grepl("[0-9]|sp[.]|aff[.]|Cf[.]|cf[.]| cf |var[.]|subsp[.]|f[.]|auct[.]",search.res$taxon),]
		if(nrow(search.res)==0){
...
get.accs.for.genes.in.clade<-function(clade.to.search,locus.list,search.terms){
	...
	for(ll in 1:length(locus.list)){
		...
		if(nrow(res.search)==1){
			#this does something different if 1 row data frame, so need transpose before making a dataframe
			res.search<-as.data.frame(t(apply(res.search,ncol(res.search),as.character)))
		}
		if(nrow(res.search)>1){
			res.search<-as.data.frame(apply(res.search,ncol(res.search),as.character))
		}
		print(class(res.search))
		write.csv(res.search,file=paste(path.for.files,"/", locus.list[ll],"_",clade.to.search,".stuff.csv",sep=""),row.names=FALSE)
		if(ll==1){
			all.search<-res.search
		}
		if(ll>1){
			print(head(all.search))
			print(head(res.search))
	...
	if(nrow(all.search)==0){
		...
		if(nrow(all.search)<=400){
			...
			for(p in 1:ceiling(nrow(all.search)/400)){
				if(p<ceiling(nrow(all.search)/400)){
					attempt<-1
					while(is.na(tax.names[((400*p)-399),]) && attempt<=5){
						if(attempt>1){
							Sys.sleep(runif(1,min=1,max=10))
						}	
						try(tax.names[((400*p)-399):(400*p),]<-ncbi_get_taxon_summary(id=all.search$NCBI.ID[((400*p)-399):(400*p)]))
						attempt<-attempt+1
					}	
				} else {
					attempt<-1
					...
					try(tax.names[((400*p)-399):nrow(all.search),]<-ncbi_get_taxon_summary(id=all.search$NCBI.ID[((400*p)-399):nrow(all.search)]))
					attempt<-attempt+1
					}
				}
			}
			...
			for(p in 1:ceiling(nrow(all.search)/400)){
				if(p<ceiling(nrow(all.search)/400)){
					...
					if(p==1){
						while(is.null(tax.output) && attempt<=5){
							if(attempt>1){
								Sys.sleep(runif(1,min=1,max=400))
							}
							try(tax.output<-tax_name(all.search$Taxon[((400*p)-399):(400*p)],get=c("Species","Genus","Tribe","Subfamily","Family"),db="ncbi",verbose=FALSE))
							attempt<-attempt+1
						}
					} else {
						attempt<-1
						while((nrow(tax.output)<(p*400)) && attempt<=5){
							if(attempt>1){
								Sys.sleep(runif(1,min=1,max=400))
							}	
							try(tax.output[((400*p)-399):(400*p),]<-tax_name(all.search$Taxon[((400*p)-399):(400*p)],get=c("Species","Genus","Tribe","Subfamily","Family"),db="ncbi",verbose=FALSE))
							attempt<-attempt+1
						}		
					}	
				} else {
					...
					while((nrow(tax.output)<nrow(all.search)) && attempt<=5){
						...
						try(tax.output[((400*p)-399):nrow(all.search),]<-tax_name(all.search$Taxon[((400*p)-399):nrow(all.search)],get=c("Species","Genus","Tribe","Subfamily","Family"),db="ncbi",verbose=FALSE))
						attempt<-attempt+1
					}
				}
			}
...
#search.terms<-c("((18S[TITL] OR 16S[TITL] OR small[TITL]) AND ribosomal[TITL] NOT (internal[TITL] OR 58s[TITL] OR 5.8S[TITL] OR endonuclease[TITL] OR mitochondrial[TITL] OR mitochondrion[TITL] OR pseudogene [TITL] OR segmented set[PROP]","((28S[TITL] OR 26S[TITL] OR 25S[TITL] OR large[TITL]) AND ribosomal[TITL] NOT (internal[TITL] OR 58s[TITL] OR 5.8S[TITL] OR endonuclease[TITL] OR mitochondrial[TITL] OR mitochondrion[TITL] OR pseudogene [TITL] OR segmented set[PROP]","(AbdA[TITL] OR abd-A[TITL] OR abdominal-A[TITL] NOT (pseudogene [TITL] OR segmented set[PROP]","(Wg[TITL] wnt-1[TITL] OR wingless[TITL] NOT (pseudogene [TITL] OR segmented set[PROP]","(LR[TITL] OR LW Rh[TITL] OR (long[TITL] AND wavelength[TITL] AND rhodopsin[TITL]) OR (long[TITL] AND wave[TITL] AND opsin[TITL]) NOT (pseudogene [TITL] OR segmented set[PROP]","(COI[TITL] OR (cytochrome[TITL] AND oxidase[TITL] AND subunit[TITL] AND I[TITL]) OR (cytochrome[TITL] AND oxidase[TITL] AND subunit[TITL] AND 1[TITL]) NOT (tRNA-Leu[TITL] OR cytochrome oxidase subunit II[TITL] OR COII[TITL] OR pseudogene [TITL] OR segmented set[PROP]","(EF-1a-F1[TITL] OR EF1aF1[TITL] NOT (pseudogene [TITL] OR shotgun[TITL] OR scaffold[TITL] OR segmented set[PROP]","(EF-1a-F2[TITL] OR EF1aF2[TITL] NOT (pseudogene [TITL] OR shotgun[TITL] OR scaffold[TITL] OR segmented set[PROP]","(arginine kinase[TITL] ArgK[TITL] NOT (pseudogene [TITL] OR shotgun[TITL] OR scaffold[TITL] OR segmented set[PROP]","(carbomoylphosphate synthase[TITL] OR carbamoyl-phosphate synthetase[TITL] OR conserved ATPase domain protein isoform 1[TITL] OR CAD[TITL] NOT (pseudogene [TITL] OR shotgun[TITL] OR scaffold[TITL] OR segmented set[PROP]","(DNA topoisomerase 1[TITL] Top1[TITL] NOT (pseudogene [TITL] OR shotgun[TITL] OR scaffold[TITL] OR segmented set[PROP]","(ultrabithorax[TITL] Ubx[TITL] NOT (pseudogene [TITL] OR shotgun[TITL] OR scaffold[TITL] OR segmented set[PROP]")

#Removed  OR segmented set[PROP] all genes and AND I[TITL] from COI...was missing sequences, and segmented set argument seems to no longer work.  Also added "long-wavelength AND rhodopsin.
#Then changed Topoisomerase to not require DNA in title and added OR topoisomerase I[TITL] OR TopI[TITL] 
#Removed space between pseudogene and [TITL]
#changed EF1aF1 to include: OR (EF-1a[TITL] AND F1 copy) OR (EF1a[TITL] AND F1 copy) OR elongation factor 1-alpha F1[TITL] OR EF1-1a-F1[TITL] OR elongation factor 1-alpha F1 copy[TITL] OR (elongation factor 1-alpha AND (EF-1a-F1 OR EF1-1a-F1 OR EF1aF1 OR F1 copy))
#changed EF1aF2 to include: OR (EF-1a[TITL] AND F2 copy) OR (EF1a[TITL] AND F2 copy) OR elongation factor 1-alpha F2[TITL] OR EF1-1a-F2[TITL] OR elongation factor 1-alpha F2 copy[TITL] OR (elongation factor 1-alpha AND (EF-1a-F2 OR EF1-1a-F2 OR EF1aF2 OR F2 copy))
#Added OR in between search terms in TOP1 and UBX
#ADDED COXI to cytochrome oxidase
#changed COX to be subunit 1[TITL] and subunit I[TITL]...see how this works...was subunit[TITL] AND 1[TITL]   and then had previously found the I[TITL] was problematic, so just did subunit[TITL]
#Remove unverified from post-cut and add to NOT part of search
search.terms<-c("((18S[TITL] OR 16S[TITL] OR small[TITL]) AND ribosomal[TITL] NOT (internal[TITL] OR 58s[TITL] OR 5.8S[TITL] OR endonuclease[TITL] OR mitochondrial[TITL] OR mitochondrion[TITL] OR pseudogene[TITL] OR UNVERIFIED[TITL]","((28S[TITL] OR 26S[TITL] OR 25S[TITL] OR large[TITL]) AND ribosomal[TITL] NOT (internal[TITL] OR 58s[TITL] OR 5.8S[TITL] OR endonuclease[TITL] OR mitochondrial[TITL] OR mitochondrion[TITL] OR pseudogene[TITL] OR UNVERIFIED[TITL]","(AbdA[TITL] OR abd-A[TITL] OR abdominal-A[TITL] NOT (pseudogene[TITL] OR UNVERIFIED[TITL]","(Wg[TITL] wnt-1[TITL] OR wingless[TITL] NOT (pseudogene[TITL] OR UNVERIFIED[TITL]","(LR[TITL] OR LW Rh[TITL] OR (long[TITL] AND wavelength[TITL] AND rhodopsin[TITL] OR long-wavelength[TITL] AND rhodopsin[TITL]) OR (long[TITL] AND wave[TITL] AND opsin[TITL]) NOT (pseudogene[TITL] OR UNVERIFIED[TITL]","(COI[TITL] OR COX1[TITL] OR (cytochrome[TITL] AND oxidase[TITL] AND subunit I[TITL]) OR (cytochrome[TITL] AND oxidase[TITL] AND subunit 1[TITL]) NOT (tRNA-Leu[TITL] OR cytochrome oxidase subunit II[TITL] OR COII[TITL] OR pseudogene[TITL] OR UNVERIFIED[TITL]","(EF-1a-F1[TITL] OR EF1aF1[TITL] OR EF1-1a-F1[TITL] OR (EF-1a[TITL] AND F1 copy) OR (EF1a[TITL] AND F1 copy) OR (elongation factor 1-alpha AND (EF-1a-F1 OR EF1-1a-F1 OR EF1aF1 OR F1 copy)) OR elongation factor 1-alpha F1[TITL] OR elongation factor 1-alpha F1 copy[TITL] NOT (pseudogene[TITL] OR shotgun[TITL] OR scaffold[TITL] OR UNVERIFIED[TITL]","(EF-1a-F2[TITL] OR EF1aF2[TITL] OR EF1-1a-F2[TITL] OR (EF-1a[TITL] AND F2 copy) OR (EF1a[TITL] AND F2 copy) OR (elongation factor 1-alpha AND (EF-1a-F2 OR EF1-1a-F2 OR EF1aF2 OR F2 copy)) OR elongation factor 1-alpha F2[TITL] OR elongation factor 1-alpha F2 copy[TITL] NOT (pseudogene[TITL] OR shotgun[TITL] OR scaffold[TITL] OR UNVERIFIED[TITL]","(arginine kinase[TITL] ArgK[TITL] NOT (pseudogene[TITL] OR shotgun[TITL] OR scaffold[TITL] OR UNVERIFIED[TITL]","(carbomoylphosphate synthase[TITL] OR carbamoyl-phosphate synthetase[TITL] OR conserved ATPase domain protein isoform 1[TITL] OR CAD[TITL] NOT (pseudogene[TITL] OR shotgun[TITL] OR scaffold[TITL] OR UNVERIFIED[TITL]","(DNA topoisomerase 1[TITL] OR topoisomerase 1[TITL] OR Top1[TITL] OR topoisomerase I[TITL] OR TopI[TITL] NOT (pseudogene[TITL] OR shotgun[TITL] OR scaffold[TITL] OR UNVERIFIED[TITL]","(ultrabithorax[TITL] OR Ubx[TITL] NOT (pseudogene[TITL] OR shotgun[TITL] OR scaffold[TITL] OR UNVERIFIED[TITL]")
...
#get subfamilies to search for
for.sfs<-read.csv(file="/Volumes/welwitschia/9nov2016_hd_v/Documents/ants_june_to_august_2016/ants.ncbi.14july2016/14july2016_antcat_aligns_fullalign_einsi_nogappy_wadds_heavilymodified_wo_results/antwiki_august2016/AntWiki_Valid_Species_w_fossils_mymods.csv",stringsAsFactors=FALSE)
for.sfs<-for.sfs[for.sfs$Status %in% "Extant",]
sort(unique(for.sfs$Subfamily))
#returns subfamilies, but also some tha tare uncertain (Condylodon)
for.sfs[for.sfs$Status %in% "Extant" & for.sfs$Subfamily %in% "uncertain",]

#clades<-"Formicidae"
clades<-sort(unique(for.sfs$Subfamily))
clades
clades[clades %in% "uncertain"]<-"Condylodon"
clades

path.for.files<-"~/Desktop/ants_for_doug"
sink(paste(path.for.files,"/","combined.acc.nos.ants.ncbi.31aug2018.txt",sep=""), append=TRUE, split=TRUE)


api_key<-"d632206e80c6cb228284295968b3620dd807"
for(cl in 1:length(clades)){
	...
	if(cl==1){
		comb.search.res<-search.res
	}
	if(cl>1){	
		comb.search.res<-rbind(comb.search.res,search.res)
	}
	if(cl==length(clades)){
		if(!is.null(comb.search.res)){
			write.acc.no.files(comb.search.res,path.for.files,"Formicidae",loci,append=FALSE)
		}		
	}
}
